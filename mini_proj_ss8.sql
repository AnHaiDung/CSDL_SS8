...
select distinct room_type, price_per_day from rooms
...
select * from bookings where check_in between '2024-01-01' and '2024-12-30';
...
select r.room_id,((b.check_out - b.check_in)*r.price_per_day) as 'tong doanh thu'
from rooms r
...
select * from rooms where (select avg(price_per_day) from rooms);
-- Hiển thị những khách chưa từng đặt phòng
select * from guests where guest_id in (select guest_id from bookings);
-- Tìm phòng được đặt nhiều lần nhất
select r.*, count(b.booking_id) as so_luot_dat
from rooms r
left join bookings b on r.room_id = b.room_id
group by r.room_id
